...
RUN apt-get update && apt-get install -y --no-install-recommends python3 python3-pip git libcairo2 && \
    ...
    python3 -m pip install --no-cache-dir transformers==4.49.0 tokenizers==0.21.1 accelerate pillow sentencepiece protobuf omegaconf fairscale matplotlib beautifulsoup4 svgpathtools cairosvg && \
    python3 -m pip install --no-cache-dir --no-deps --ignore-requires-python git+https://github.com/joanrod/star-vector.git && \
    sed -i 's/attn_implementation="flash_attention_2"/attn_implementation="sdpa"/' /usr/local/lib/python3.10/dist-packages/starvector/model/llm/starcoder2.py
